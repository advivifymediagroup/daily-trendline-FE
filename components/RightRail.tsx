        ...
        <div className="flex flex-wrap gap-2.5">
          {DEFAULT_NAV.map((link) => (
            <Link
              key={link.url}
              href={link.url}
              className="border hairline bg-stone-50 px-3.5 py-2 text-sm font-medium text-slate-700 no-underline transition-colors hover:bg-slate-900 hover:text-white dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-100 dark:hover:text-slate-900"
            >
              {link.text}
            </Link>
          ))}
